...
> **BE-06 — отступление от буквы ТЗ (осознанное).** ТЗ называет плагин
> `analysis-ru`; официального плагина с таким именем в Elasticsearch нет, а
> «морфология» — сторонний неподдерживаемый под ES 8.x плагин. Используем
> встроенный анализатор `russian` (snowball-стемминг + стоп-слова) — русский
> полнотекстовый поиск работает без внешних зависимостей. Апгрейд-путь при
> необходимости лемматизации: плагин `russian_morphology` (см. коммент в
> `backend/app/services/es.py`).
